...
export function useDatabaseColecao(idUser) {
            ...
            (snapshot) => {
                ...
                if (snapshot.exists()) {
                    ...
                    for (let key in colecoesData) {
                        ...
                        data.push({ createAt: item.createAt, key: item.key, name: item.name, updateAt: item.updateAt, ordeBy: item.ordeBy ? item.ordeBy : colecoesData.length + 1 })
                    }
    ...
    async function updateColecao(name, id, orderBy = null) {
        let data = {};
        if (orderBy !== null) {
            data = {
    ...
    return { colecoes, setColecoes, init, addColecao, getColecao, deleteColecao, updateColecao }
